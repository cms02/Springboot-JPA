...
import com.cms.blog.model.KakaoProfile;
import com.cms.blog.model.OAuthToken;
...
public class UserController {
	...
	public String kakaoCallback(String code) {   //@ResponseBody -> Data를 리턴해주는 컨트롤러 함수
		...
		params.add("client_id", "53c0e12c131cce3c420a6060a2c68c99");
...
//		UUID garbagePassword = UUID.randomUUID();  //UUID란 -> 중복되지 않는 어떤 특정 값을 만들어내는 알고리즘
		System.out.println("블로그서버	패스워드: "	 + cmsKey);
		
		User kakaoUser = User.builder()
				.username(kakaoProfile.getKakao_account().getEmail()+ "_" + kakaoProfile.getId())
				.password(cmsKey.toString())
		...
		//로그인처리
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(kakaoUser.getUsername(), cmsKey));
